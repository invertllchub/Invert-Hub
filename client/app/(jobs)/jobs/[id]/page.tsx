    ...
    return (
        <div className='w-full'>
            ...
            <div className="relative w-full h-[80vh]">
                ...
                >
                    <h1 className="w-7/12 text-start mx-auto text-5xl font-bold drop-shadow-lg">
                        {job?.title} - {job?.location}
                    </h1>
                    <div className='w-7/12 mx-auto mt-5 flex items-start gap-6'>
                        <p className='flex gap-2'><Hourglass size={20}/> {job?.employmentType}</p>
                        <p className='flex gap-2'><MapPin size={20}/> {job?.location}</p>
                        <p className='flex gap-2'><Calendar size={20}/> {job?.datePosted}</p>
                        <p className='flex gap-2'><Clock size={20}/> {job?.closingDate}</p>
                    </div>
                    <div className='w-7/12 mx-auto mt-10'>
                        <p className='text-xl font-semibold'>We are currently looking for an Advanced {job?.title} to join us.</p>
                    </div>
                </div>
            </div>

            <div className="py-4 mb-2">
                <div className="w-9/12 flex items-center gap-4 mx-auto">
                    ...
                    <button
                        ...
                        ${showForm ? 'rounded-none shadow-none bg-transparent text-black border-b-2' 
