...
// Types
import { SwiperProps } from "@/types/project";
import { Swiper4Props } from "@/types/careers";
    ...
    >
      {project.facts.map((fact, i) => (
        <SwiperSlide key={fact.id}>
          {fact.type === "text" ? (
            <div
              className={`bg-gradient-to-r h-[60vh] flex flex-col items-start justify-end gap-3 p-4
                    ${
                      i % 2 === 0
        ...
        <SwiperSlide key={i}>
          <div className="w-full h-full md:h-[90vh] flex flex-col-reverse md:flex-row items-center justify-between gap-4">
            <div className="w-full md:w-6/12 h-6/12 md:h-full align-text-top md:p-4">
              ...
              <h1 className="text-xl md:text-5xl font-semiblod">
                {swipe.title}
              </h1>
              <h3 className="text-bold mt-3 md:mt-6 text-sm">
                {swipe.description}
              </h3>
              <div className="flex gap-4 mt-4 md:mt-8">
                <button className="swiper-button-prev bg-gray-200 hover:bg-gray-300 rounded-full p-2 md:p-3 shadow cursor-pointer">
                  ←
                </button>
                <button className="swiper-button-next bg-gray-200 hover:bg-gray-300 rounded-full p-2 md:p-3 shadow cursor-pointer">
                  →
                </button>
              </div>
            </div>
  ...
  return (
    ...
    >
      {locations.map((location, i) => (
        <SwiperSlide key={i}>
          <div
            className={`bg-gradient-to-r h-[50vh] md:h-[70vh] flex flex-col items-start justify-end p-4
                ${
                  i % 2 === 0
